...
namespace ProgrammersBlog.Mvc.Areas.Admin.Controllers
{
    ...
    public class RoleController : BaseController
    {
        ...
        {
            var roles = await _roleManager.Roles.ToListAsync();
            var roleListDto = System.Text.Json.JsonSerializer.Serialize(new RoleListDto
            {
                Roles = roles
            });
        ...
        public async Task<IActionResult> Assign(UserRoleAssignDto userRoleAssignDto)
        {
            if (ModelState.IsValid)
            {
                ...
                var userRoleAssignAjaxViewModel = JsonSerializer.Serialize(new UserRoleAssignAjaxViewModel
                {
                    ...
                    {
                        User=user,
                        Message=$"{user.UserName} kullanicisina ait rol atama islemi basaiyla tamamlanmistir.",
