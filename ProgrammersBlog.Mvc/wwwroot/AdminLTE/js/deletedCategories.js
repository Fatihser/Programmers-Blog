$(document).ready(function () {
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            Swal.fire({
                title: 'Arsivden geri getirmek istediğinize emin misiniz?',
                text: `${categoryName} içerikli yorum arsivden geri getirilecektir.!`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Evet, arsivden geri getirmek istiyorum.',
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            let categoryName = tableRow.find('td:eq(1)').text();
            Swal.fire({
                title: 'Kalici olarak silmek istediğinize emin misiniz?',
                text: `${categoryName} adli kategory kalici olarak silinecektir getirilecektir.!`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Evet, kalici olarak silmek istiyorum.',
                cancelButtonText: 'Hayır, istemiyorum.'
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (data) {
                            const hardDeletedCategoryResult = jQuery.parseJSON(data);
                            console.log(hardDeletedCategoryResult);
                            if (hardDeletedCategoryResult.ResultStatus === 0) {
                                Swal.fire(
                                    'Kalici olarak silindi!',
                                    `${hardDeletedCategoryResult.Message}`,
                                    'success'
                                );
                                ...
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Başarısız İşlem!',
                                    text: `${hardDeletedCategoryResult.Message}`,
                                });
                            }
                        },
                        error: function (err) {
